...
- **2026-05-24 → 2026-05-25 — Job source expansion + cleanup (PRs #117–#122).** Shipped Workable adapter (PR #117), IAI custom-WordPress adapter (~470 IL defense jobs, PR #118), and Jooble multi-source aggregator (PR #119). Then uncapped Workday (3→11 IL search-text cities, surfaces previously-invisible NVIDIA Ra'anana / Intel Haifa / Mobileye Jerusalem / AWS Yokneam jobs) + added Workday/Jooble truncation alerts (PR #120). **Disabled Jooble (PR #120)** — landscape probe revealed the API has zero IL coverage (`location: "Israel"` matched US towns in OH/IL/IN; unknown locations silently fall back to global). Fixed all 6 Workable api_urls in registry + corrected Autofleet slug "j" → "autofleet" unlocking 9 IL jobs (PR #121). Added reusable ATS-discovery probe (PR #122) — careers-page HTML crawl that finds slug-encoded Greenhouse/Lever/Ashby/Workable URLs and validates each against the live API. First-run yield on 260 "unknown" registry entries: 0 hits (the unknowns are JS-rendered SPAs or custom/dead boards); tool preserved for future runs against fresh candidate lists (YC IL / Built In Israel). Lesson logged: any new job source must inspect returned rows' actual `.location` field before shipping. Final pipeline: 10 ATS sources (greenhouse, lever, ashby, workday, smartrecruiters, comeet, successfactors, workable, iai, +disabled jooble), 832 registered companies, 297 actively producing 2,817 active IL jobs.
- **2026-05-08 — Docs catch-up + living source of truth (PR #36).** `PROJECT_INSTRUCTIONS.md` established as the index/onboarding doc — cross-references `CLAUDE.md` / `ROADMAP.md` / `tasks/lessons.md` / `docs/research/`. Brings any new collaborator (Isaac) fully up to speed in one read: where we are, all 15 edge functions, all 5 voice-rules constants, key files, sprint status, Isaac's task list, how to work with Claude Code (ask-don't-tell, decision checkpoints, full-CI-before-push), tools we use. ROADMAP Done section caught up through PRs #20-#35. PR template gained a checkbox enforcing the update rule.
- **2026-05-06 → 2026-05-08 — Wk 3 LinkedIn command center: Networking tab Phase 4 PR B (PR #35).** Outreach Conversation Coach. 8 outreach goals (recruiter / hiring manager / alumni / informational interview / thank-you / reconnect-dormant / referral / recommendation), multi-turn coaching, warm-up-vs-ask judgment for multi-step goals (dormant-relationship referrals correctly emit `warm_up_advice` "send reconnection first, ask in turn 2-3"). Schema: 1 row per (user, goal, target_person), JSONB `message_thread`, 4-policy RLS. Edge function with two modes (new / continue). Programmatic anti-pattern detection in post-process — model still slips "I hope you're doing well" despite hard-rule injection, so warning chips surface to user. NetworkingPrinciples relocated to Resources page (Resources accordion now supports optional `component` field on guide entries).
...
- **2026-07-23 — Onboarding V2 arc, PR-2 (branch `eli/onboarding-v2-selfheal-tutorial`, PR #691, HELD).** Self-heal (b): new `src/lib/careerAnalysis.js` `runCareerAnalysisAndReplaceRoles(...)` consolidates the career-analysis invoke + `replace_career_roles` RPC across handleSurveyNext / Roadmap generate / V2 finalise background-fire (V2 had no roadmap producer). Plus the OnboardingTutorial returns after the V2 springboard with an always-visible "Skip tour" for fresh users. Gates green; acceptance drive passed (career_roles produced via the shared helper, tutorial+skip, situations array on the inferred event). Held for hub. Sequence: #683 → #688 (PR-1) → **PR-2 (#691)** → Phase 1 restyle → Phase 2 UX → flag flip LAST.
...
- **[BUG] Story extraction never fills `situation` + `task` (only `action` + `result`).** All 15 stories for user 4b243f3a (both `manual_quick_add` and `conversation` sources) have empty `situation`/`task` (length 0) while `action`/`result` are populated. This is an **extraction-prompt gap** in `extract-story-from-text` / the chat story-capture prompt, not a render bug. Surfaced alongside the 2026-06-16 Story Bank cache-projection fix (PR `eli/storybank-cache-fix`): that fix makes `title` + `action` + `result` render correctly, but the Situation and Task STAR rows will stay blank until extraction is taught to populate them. Fix: extend the extraction prompt to emit all four STAR fields, and consider a one-off backfill re-extract for existing rows. **Do NOT touch `ai-chat` / #330 as part of this** — separate function + prompt.
