...
// at the top of Roadmap.jsx and cascades to RoleCard + TierQuadrantGrid.
...
/* Per-tier accent variables — used by RoleCard, TierQuadrantGrid, badges */
.rm-tier-green { --rm-tier-accent: var(--rm-success); --rm-tier-tint: var(--rm-success-tint); --rm-tier-border: rgba(29,117,86,0.3); }
.rm-tier-gray  { --rm-tier-accent: var(--rm-ink-soft); --rm-tier-tint: var(--rm-bg-tinted); --rm-tier-border: rgba(82,84,90,0.25); }
.rm-tier-amber { --rm-tier-accent: var(--rm-warning); --rm-tier-tint: var(--rm-warning-tint); --rm-tier-border: rgba(184,132,28,0.3); }

/* Tier badge — number circle in tier color */
.rm-tier-badge { display: inline-flex; align-items: center; justify-content: center; width: 22px; height: 22px; border-radius: 50%; font-family: var(--rm-font); font-size: 11.5px; font-weight: 700; color: white; background: var(--rm-tier-accent, var(--rm-ink-soft)); }
.rm-tier-pill { display: inline-flex; align-items: center; gap: 7px; padding: 4px 10px 4px 6px; border-radius: 100px; background: var(--rm-tier-tint, var(--rm-bg-tinted)); color: var(--rm-tier-accent, var(--rm-ink-soft)); font-size: 12px; font-weight: 600; line-height: 1; }
...
.rm-role-card::before { content: ""; position: absolute; top: 0; left: 0; right: 0; height: 3px; background: var(--rm-tier-accent, var(--rm-ink-soft)); }
...
/* Tier breakdown bars — coral fill on the weak axis to draw the eye */
...
.rm-quadrant-cell[data-tier="green"] { background: var(--rm-success-tint); border: 2px solid var(--rm-success); }
.rm-quadrant-cell[data-tier="gray"]  { background: var(--rm-bg-tinted);   border: 1px solid var(--rm-line); }
.rm-quadrant-cell[data-tier="amber"] { background: var(--rm-warning-tint); border: 1px solid rgba(184,132,28,0.4); }
.rm-quadrant-cell[data-tier="empty"] { background: var(--rm-card); border: 1px dashed var(--rm-line); }
...
.rm-quadrant-cell[data-tier="green"] .rm-quadrant-tag { color: var(--rm-success); }
.rm-quadrant-cell[data-tier="amber"] .rm-quadrant-tag { color: var(--rm-warning); }
.rm-quadrant-cell[data-tier="gray"]  .rm-quadrant-tag { color: var(--rm-ink-soft); }
.rm-quadrant-cell[data-tier="empty"] .rm-quadrant-tag { color: var(--rm-ink-faded); }
.rm-quadrant-label { font-size: 11px; margin-top: 2px; }
.rm-quadrant-cell[data-tier="green"] .rm-quadrant-label { color: #0F4E36; }
.rm-quadrant-cell[data-tier="amber"] .rm-quadrant-label { color: #6B4E0F; }
.rm-quadrant-cell[data-tier="gray"]  .rm-quadrant-label { color: var(--rm-ink-soft); }
.rm-quadrant-cell[data-tier="empty"] .rm-quadrant-label { color: var(--rm-ink-faded); font-style: italic; }
