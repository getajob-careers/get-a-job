...
// PostImageUpload — handles file selection, upload to the
// linkedin_post_images Supabase Storage bucket, and persistence of the
// resulting signed URL on linkedin_posts.image_url.
//
// Storage path: {user_id}/{post_id}_{timestamp}_{filename}
// Bucket RLS enforces "first folder segment must equal auth.uid()".
// Signed URLs are issued for ~10 years (same pattern as profiles.resume_url)
// so we can persist them on the row instead of re-fetching per render.
...
export default function PostImageUpload({ postId, imageUrl, onChange }) {
  ...
  const handleRemove = async () => {
    ...
    setRemoving(true);
    try {
      // We only clear the DB pointer — the storage object stays, since
      // we'd need to parse the signed URL back into a path to delete it
      // and the cost of orphaned images is trivial (5MB cap × low volume).
      // A periodic cleanup job can sweep these later if needed.
  ...
  if (imageUrl) {
    return (
      <div className="li-image-thumb">
        <img src={imageUrl} alt="Attached" />
        <button
          type="button"
          onClick={handleRemove}
          disabled={removing}
          className="li-image-thumb-remove"
