...
// PR 3J-B — restyled on rd-* tokens. Restyle-only on behavior: all
// per-type defaults, validation thresholds, and submit-gating
// (canSubmit) preserved byte-for-byte. The live multi-field forms are
// kept verbatim — the mockup simplifies visually but per Eli's ruling
// we do NOT remove fields.
//
// Eli's PR #32 field decisions still apply per type:
//   - project: project_name, context, what_you_built (multi-line),
//              outcome (free-text per Eli), portfolio_link (optional)
//   - lessons: source_type, source_name, lessons[3-5] (chip adder)
//   - milestone: milestone_type, the_thing, people_to_thank[1+]
//                (Add another, max 5 — encourage not require),
//                whats_next (optional)
...
export default function PostComposeForm({ postType, inputs, onChange, onBack, onGenerate, generating }) {
    ...
    return (
      <FormShell postType={postType} onBack={onBack} onGenerate={onGenerate} generating={generating}
        ...
        <Field label="Project name" required>
          <Input value={inputs.project_name || ""} onChange={(e) => set("project_name", e.target.value)} placeholder="e.g. VIP onboarding redesign at Guardio" className={RD_INPUT_CLS} />
        </Field>
        ...
        <Field label="Portfolio / demo link" hint="Optional. Adds a 'see the work' destination at the end.">
          <Input value={inputs.portfolio_link || ""} onChange={(e) => set("portfolio_link", e.target.value)} placeholder="https://..." className={RD_INPUT_CLS} />
        </Field>
      </FormShell>
    );
  }
    ...
    return (
      <FormShell postType={postType} onBack={onBack} onGenerate={onGenerate} generating={generating} canSubmit={canSubmit}>
        ...
        <Field label="Source name" required hint="e.g. 'Customer Discovery class with Prof. Lee' or 'First quarter as Customer Success Specialist'.">
          <Input value={inputs.source_name || ""} onChange={(e) => set("source_name", e.target.value)} placeholder="What did you do / where did you learn this" className={RD_INPUT_CLS} />
        </Field>
        <div>
          ...
          <div className="space-y-2">
            {lessons.map((l, i) => (
              <div key={i} className="flex gap-2 items-start">
                <span className="text-[11px] font-mono text-rd-text-tertiary mt-2 w-5">{i + 1}.</span>
                <textarea
                  value={l}
                  ...
                  rows={2}
                  className={`flex-1 ${RD_TEXTAREA_CLS}`}
                />
            ...
            {lessons.length < 5 && (
              <button type="button" onClick={addLesson} className="inline-flex items-center gap-1 text-[12px] text-rd-text-secondary hover:text-rd-text">
                <Plus className="w-3 h-3" />
                Add another lesson
              </button>
            )}
          </div>
          <p className="text-[10px] text-rd-text-tertiary mt-2">{filled} of 3–5 filled</p>
        </div>
      </FormShell>
    );
  }
    ...
    return (
      <FormShell postType={postType} onBack={onBack} onGenerate={onGenerate} generating={generating} canSubmit={canSubmit}>
        ...
        <Field label="The specific thing" required hint="e.g. 'Customer Success Specialist offer at Acme' or 'Bachelor's in Business Administration from State University'. Specific beats generic.">
          <Input value={inputs.the_thing || ""} onChange={(e) => set("the_thing", e.target.value)} placeholder="What just happened" className={RD_INPUT_CLS} />
        </Field>
        <div>
          ...
          <div className="space-y-2">
            ...
            {people.length < 5 && (
              <button type="button" onClick={addPerson} className="inline-flex items-center gap-1 text-[12px] text-rd-text-secondary hover:text-rd-text">
                <Plus className="w-3 h-3" />
                Add another person
              </button>
            )}
    ...
    return (
      <FormShell postType={postType} onBack={onBack} onGenerate={onGenerate} generating={generating} canSubmit={canSubmit}>
        <Field label="Event name" required>
          <Input value={inputs.event_name || ""} onChange={(e) => set("event_name", e.target.value)} placeholder="e.g. TechCrunch Disrupt 2026" className={RD_INPUT_CLS} />
        </Field>
        <Field label="What you did at the event" required hint="e.g. 'Team lead', 'Backend dev', 'Pitch presenter', 'Volunteer organizer'">
          <Input value={inputs.role_played || ""} onChange={(e) => set("role_played", e.target.value)} placeholder="Your role" className={RD_INPUT_CLS} />
        </Field>
        ...
        <div>
          <label className="block text-[11px] font-display font-semibold text-rd-text mb-1">Team members to tag</label>
          <p className="text-[11px] text-rd-text-secondary mb-2">
          ...
          <div className="space-y-2">
            ...
            {team.length < 10 && (
              <button type="button" onClick={addTeam} className="inline-flex items-center gap-1 text-[12px] text-rd-text-secondary hover:text-rd-text">
                <Plus className="w-3 h-3" />
        ...
        <Field label="Key lesson" hint="Optional ≤200 chars. One takeaway worth carrying forward — lifts the post's saveability if the lesson is concrete.">
          <textarea value={inputs.key_lesson || ""} onChange={(e) => set("key_lesson", e.target.value.slice(0, 200))}
                    ...
                    rows={2} className={RD_TEXTAREA_CLS} />
          <p className="text-[10px] text-rd-text-tertiary mt-1 text-right">{(inputs.key_lesson || "").length}/200</p>
        </Field>
      </FormShell>
    );
  }
  ...
  if (postType === "observation") {
    ...
    return (
      <FormShell postType={postType} onBack={onBack} onGenerate={onGenerate} generating={generating} canSubmit={canSubmit}>
        <div className="rounded-[14px] px-4 py-3 text-[11.5px] leading-snug bg-rd-golden-tint border border-rd-golden/40 text-rd-golden-dark">
          <strong className="font-display font-bold">Heads-up:</strong> Observation posts are the highest-risk format for early-career accounts. Without a concrete <em>specific example</em> anchoring your authority, this post will read as overreach. The framework requires you to provide one.
        </div>
    ...
    return (
      <FormShell postType={postType} onBack={onBack} onGenerate={onGenerate} generating={generating} canSubmit={canSubmit}>
        ...
        <Field label="Why posting" required hint="The structural intent — drives how the AI shapes the post.">
          ...
          <p className="text-[11px] text-rd-text-tertiary mt-1.5 italic">
...
function FormShell({ children, onBack, onGenerate, generating, canSubmit }) {
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-rd-border rounded-[14px] p-5 space-y-4 shadow-rd">
        {children}
      </div>
