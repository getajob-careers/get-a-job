...
export default function CanvasJobModal({
  ...
  return (
    ...
    >
      <div
        className="bg-rd-bg-card rd-r-lg w-full max-w-[540px] max-h-[88vh] flex flex-col overflow-hidden shadow-[0_24px_60px_rgba(40,25,10,0.28)]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="px-5 pt-4 pb-3.5 border-b border-rd-border flex items-start gap-3">
          <span
            className="flex-shrink-0 inline-flex items-center justify-center w-[46px] h-[46px] rd-r-sm font-display font-extrabold rd-t-display-s"
            style={{
          ...
          <div className="flex-1 min-w-0">
            <h2 className="font-display font-extrabold rd-t-display-m leading-[1.15] text-rd-text break-words">
              {job.title}
            </h2>
            <p className="rd-t-body-s text-rd-text-secondary mt-0.5 break-words">
              {subLine}
            </p>
            {job.is_agency && (
              <span className="inline-flex items-center mt-1 rd-t-micro bg-rd-golden-tint text-rd-golden-dark rounded-full px-1.5 py-0.5">
                via staffing agency
              </span>
            )}
          </div>
          {d.bandMeta && (
            ...
            >
              <span className="font-extrabold rd-t-body-s">
                {d.bandMeta.label}
              </span>
              {d.attainPct != null && (
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Close"
            className="flex-shrink-0 p-1 rd-r-xs text-rd-text-tertiary hover:text-rd-text hover:bg-rd-bg-soft transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="px-5 py-4 overflow-y-auto">
          {d.reasonText && (
            <p className="rd-t-body-s text-rd-text-secondary leading-[1.55] mb-4">
              {d.reasonText}
            </p>
          )}
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {d.matchedSkills.map((s, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center gap-1 rd-t-body-s px-2.5 py-0.5 rounded-full bg-rd-teal-tint text-rd-teal-dark"
                  >
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {d.missingCoreSkills.map((s, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center gap-1 rd-t-body-s px-2.5 py-0.5 rounded-full bg-rd-bg-soft text-rd-text-tertiary border border-rd-border"
                  >
          ...
          <p className="rd-t-body-s text-rd-text-secondary leading-[1.65] whitespace-pre-line">
        ...
        <div className="px-5 py-3 border-t border-rd-border bg-rd-bg-page flex justify-end gap-2.5">
          <button
            ...
            onClick={() => onTrack?.(job)}
            disabled={tracked}
            className="inline-flex items-center gap-1.5 font-display font-semibold rd-t-body-s rounded-full px-4 py-2 transition-colors disabled:cursor-not-allowed"
            style={
              tracked
                ? {
