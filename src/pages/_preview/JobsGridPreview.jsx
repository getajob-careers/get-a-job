...
const FIXTURE = [
  {
    ...
    score: {
      ...
      reasoning: {
        strengths: ["Strong skills match", "Right seniority for you"],
      },
    },
  ...
  },
  {
    ...
    score: {
      ...
      attainability_score: 0.78,
      signals: {
        matched_skills: ["sql", "data_analysis"],
        missing_core_skills: ["ab_testing"],
      },
      reasoning: { strengths: ["Your SQL + analytics line up well"] },
    },
  ...
  },
  {
    ...
    score: {
      ...
      signals: {
        matched_skills: ["process_design"],
        missing_core_skills: ["financial_modeling", "sql"],
      },
      reasoning: { strengths: ["Adjacent to your operations experience"] },
    },
  ...
  },
  {
    job: {
      ...
      company_slug: "lightricks",
      location_city: "Jerusalem",
      is_remote: true,
      seniority: "mid",
      years_experience_min: 2,
      years_experience_max: 5,
      date_posted: new Date(Date.now() - 4 * 86400000).toISOString(),
      apply_url: "https://example.com",
    },
    score: {
      fit_score: 0.81,
      track: "track_2",
      attainability_band: "strong",
      attainability_score: 0.81,
      signals: {
        matched_skills: ["process_design", "data_analysis"],
        missing_core_skills: ["tooling_admin"],
      },
      reasoning: { strengths: ["Process + data skills are a direct match"] },
    },
  ...
  },
  {
    job: {
      id: "g5",
      title: "Business Analyst, Revenue",
      company_name: "Gong",
      company_slug: "gong",
    ...
    score: {
      ...
      signals: {
        ...
        missing_core_skills: ["salesforce"],
      },
      reasoning: { strengths: ["Analytics foundation fits the role"] },
    },
  ...
  },
  {
    job: {
      ...
      company_name: "Acme Stealth",
      company_slug: "no-match",
    ...
    score: {
      fit_score: 0.48,
      track: "track_3",
      attainability_band: "reach",
      attainability_score: 0.48,
      signals: {
...
export default function JobsGridPreview() {
  ...
  const queryClient = useMemo(() => {
    ...
    for (const f of FIXTURE)
      qc.setQueryData(jobDescriptionKey(f.job.id), f.desc);
    return qc;
  }, []);
        ...
        <div className="min-h-screen bg-rd-bg-page p-6">
          <div className="max-w-[1000px] mx-auto">
            ...
            <p className="text-[12.5px] text-rd-text-secondary mb-3">
              ...
              Last card has no logo match → letter placeholder. Load with
              ?next=1 to see the flag-on hover-actions.
            </p>
            {/* CV-gen theater harness: the running / ready states only appear during
                a live generation, so they aren't diff-reviewable. These inject the
                shared cvGenerationJob store for card 1 so both states (and the
                card->modal sharing) are eyeball-reviewable without a real generation. */}
            <div className="flex items-center gap-2 mb-5">
              <span className="text-[11px] text-rd-text-secondary font-mono">
                card 1 CV-gen:
              </span>
              <button
                type="button"
                onClick={() => startCvGeneration(jobKeyOf(FIXTURE[0].job))}
                className="text-[11px] font-semibold rounded-full px-2.5 py-1 bg-rd-bg-soft text-rd-text hover:text-rd-primary-dark transition-colors"
              >
                running
              </button>
              <button
