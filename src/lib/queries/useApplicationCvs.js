...
function toCvOption(row) {
  ...
  if (row.is_master) {
    return {
      id: row.id, isMaster: true, tag: "Master",
      label: "Master CV", sub: "Your full, untailored CV",
      role: null, company: null, cvUrl: row.cv_url ?? null, applicationId: null,
    };
  }
  return {
    id: row.id, isMaster: false, tag: "Tailored",
    label: role || "Tailored CV",
    sub: company ? `${company} · tailored copy` : "tailored copy",
    role, company, cvUrl: row.cv_url ?? null, applicationId: row.application_id ?? null,
  };
}

const fetchApplicationCvsWithTimeout = withDbTimeout(fetchApplicationCvs, "application_cvs");
