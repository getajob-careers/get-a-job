// scripts/preview-tasks.mjs — Tasks preview pipeline (PR 3H).
//
// Same shape as the prior runners: build prod, boot vite preview,
// confirm /_preview/tasks/* is unreachable in prod; then start vite
// DEV and screenshot each fixture across desktop + mobile.
// Output → docs/design/redesign/previews/tasks-3h.pdf.
...
const outPdf = resolve(outDir, "tasks-3h.pdf");

const PORT = 4193;
const BASE = `http://localhost:${PORT}`;

const FIXTURE_PATH = resolve(repoRoot, "src/pages/_preview/fixtures/tasks.js");
...
async function verifyProd404() {
  console.log("Verifying prod build /_preview/tasks is unreachable…");
  ...
  try {
    ...
    await page.goto(`http://localhost:${prodPort}/_preview/tasks/tasks-populated`, {
      waitUntil: "networkidle",
    });
    const bodyText = await page.evaluate(() => document.body.innerText);
    await browser.close();
    // Tasks-specific markers. If any appear in prod, the DEV gate has
    // failed.
    const MARKERS = [
      "Your next moves.",
    ...
    if (MARKERS.some((m) => bodyText.includes(m))) {
      throw new Error(
        "Prod /_preview/tasks check FAILED — Tasks rendered in production. The DEV gate isn't working.",
      );
    }
    console.log("Prod build /_preview/tasks unreachable (falls through to AuthenticatedApp -> /login).");
  } finally {
...
async function main() {
  ...
  try {
    ...
    if (ids.length === 0) throw new Error("No tasks fixtures loaded.");
    console.log(`Loaded ${ids.length} tasks fixtures: ${ids.join(", ")}`);
    ...
    for (const vp of VIEWPORTS) {
      ...
      for (const id of ids) {
        ...
        const url = `${BASE}/_preview/tasks/${id}`;
        await page.goto(url, { waitUntil: "domcontentloaded" });
        await page.waitForLoadState("networkidle");
        // Beat for the post-mount DOM-action driver to fire its two
        // passes (filter pill / date-editor open / generate-error
        // simulation).
