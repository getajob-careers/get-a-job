// scripts/preview-internship.mjs — Internship (PR 3L) preview pipeline.
// Loads all fixtures with ids starting with `internship-` and writes
// docs/design/redesign/previews/internship-3l.pdf.
...
const outPdf = resolve(outDir, "internship-3l.pdf");

const PORT = 4199;
const BASE = `http://localhost:${PORT}`;

const FIXTURE_PATH = resolve(repoRoot, "src/pages/_preview/fixtures/internship.js");
...
function loadFixtureIds() {
  ...
  while ((m = re.exec(src)) !== null) {
    if (!m[1].startsWith("internship-")) continue;
    ids.push(m[1]);
    labels[m[1]] = m[2];
  }
  return { ids, labels };
...
async function verifyProd404() {
  console.log("Verifying prod build /_preview/internship is unreachable…");
  ...
  try {
    ...
    await page.goto(
      `http://localhost:${prodPort}/_preview/internship/internship-empty-pipeline`,
    ...
    if (MARKERS.some((m) => bodyText.includes(m))) {
      throw new Error(
        "Prod /_preview/internship check FAILED — Internship rendered in production. The DEV gate isn't working.",
      );
    }
    console.log("Prod build /_preview/internship unreachable (falls through to AuthenticatedApp -> /login).");
  } finally {
...
async function main() {
  ...
  try {
    ...
    if (ids.length === 0) throw new Error("No internship-* fixtures loaded.");
    console.log(`Loaded ${ids.length} internship fixtures: ${ids.join(", ")}`);
    ...
    for (const vp of VIEWPORTS) {
      ...
      for (const id of ids) {
        ...
        const url = `${BASE}/_preview/internship/${id}`;
