// scripts/preview-linkedin-posts.mjs — LinkedIn 3J-B (Posts) preview
// pipeline. Loads only fixtures whose ids start with `linkedin-posts-`
// and writes docs/design/redesign/previews/linkedin-3jb.pdf.
...
const outPdf = resolve(outDir, "linkedin-3jb.pdf");
...
function loadFixtureIds() {
  ...
  while ((m = re.exec(src)) !== null) {
    if (!m[1].startsWith("linkedin-posts-")) continue; // 3J-B subset only
    ids.push(m[1]);
    labels[m[1]] = m[2];
  }
  return { ids, labels };
...
async function verifyProd404() {
  ...
  try {
    ...
    await page.goto(`http://localhost:${prodPort}/_preview/linkedin/linkedin-posts-idle-empty`, {
...
async function main() {
  ...
  try {
    ...
    if (ids.length === 0) throw new Error("No linkedin-posts-* fixtures loaded.");
    console.log(`Loaded ${ids.length} posts fixtures: ${ids.join(", ")}`);
