// scripts/preview-jobs.mjs — Jobs preview pipeline (PR 3D).
//
// Same shape as the onboarding + shell + home + roadmap runners: build
// the prod bundle, boot `vite preview`, confirm /_preview/jobs/* is
// unreachable in prod; then start vite DEV and screenshot each fixture
// across desktop + mobile. Output → docs/design/redesign/previews/jobs-3d.pdf.
...
const outPdf = resolve(outDir, "jobs-3d.pdf");

const PORT = 4181;
const BASE = `http://localhost:${PORT}`;

const FIXTURE_PATH = resolve(repoRoot, "src/pages/_preview/fixtures/jobs.js");
...
async function verifyProd404() {
  console.log("Verifying prod build /_preview/jobs is unreachable…");
  ...
  try {
    ...
    await page.goto(`http://localhost:${prodPort}/_preview/jobs/jobs-populated`, {
      waitUntil: "networkidle",
    });
    const bodyText = await page.evaluate(() => document.body.innerText);
    await browser.close();
    // Match the Jobs-specific hero copy. If this string appears in
    // prod, the DEV gate failed and the route is reachable.
    const MARKERS = ["Live roles, scored against your tracks", "Search a role or company"];
    if (MARKERS.some((m) => bodyText.includes(m))) {
      throw new Error(
        "Prod /_preview/jobs check FAILED — Jobs rendered in production. The DEV gate isn't working.",
      );
    }
    console.log("✅ Prod build /_preview/jobs unreachable (falls through to AuthenticatedApp → /login).");
  } finally {
...
async function main() {
  ...
  try {
    ...
    if (ids.length === 0) throw new Error("No jobs fixtures loaded.");
    console.log(`Loaded ${ids.length} jobs fixtures: ${ids.join(", ")}`);
    ...
    for (const vp of VIEWPORTS) {
      ...
      for (const id of ids) {
        ...
        const url = `${BASE}/_preview/jobs/${id}`;
        await page.goto(url, { waitUntil: "domcontentloaded" });
        await page.waitForLoadState("networkidle");
        // Beat for the supabase stub + cache seed + URL flag redirect.
