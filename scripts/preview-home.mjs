// scripts/preview-home.mjs — Home preview pipeline (PR 3B).
//
// Same shape as the onboarding + shell runners: build the prod bundle,
// boot `vite preview`, confirm /_preview/home/* is unreachable in prod;
// then start vite DEV and screenshot each fixture across desktop + mobile.
// Output → docs/design/redesign/previews/home-3b.pdf.
...
const outPdf = resolve(outDir, "home-3b.pdf");

const PORT = 4177;
const BASE = `http://localhost:${PORT}`;

const FIXTURE_PATH = resolve(repoRoot, "src/pages/_preview/fixtures/home.js");
...
async function verifyProd404() {
  console.log("Verifying prod build /_preview/home is unreachable…");
  ...
  try {
    ...
    await page.goto(`http://localhost:${prodPort}/_preview/home/home-empty`, {
      waitUntil: "networkidle",
    });
    const bodyText = await page.evaluate(() => document.body.innerText);
    await browser.close();
    // Match the Home hero's "live job matches" / "Matches incoming"
    // copy — Home itself never renders on /login (the fall-through),
    // so any of these strings appearing in prod means the gate failed.
    const MARKERS = ["live job matches for you", "Matches incoming"];
    if (MARKERS.some((m) => bodyText.includes(m))) {
      throw new Error(
        "Prod /_preview/home check FAILED — Home rendered in production. The DEV gate isn't working.",
      );
    }
    console.log("✅ Prod build /_preview/home unreachable (falls through to AuthenticatedApp → /login).");
  } finally {
...
async function main() {
  ...
  try {
    ...
    if (ids.length === 0) throw new Error("No home fixtures loaded.");
    console.log(`Loaded ${ids.length} home fixtures: ${ids.join(", ")}`);
    ...
    for (const vp of VIEWPORTS) {
      ...
      for (const id of ids) {
        ...
        const url = `${BASE}/_preview/home/${id}`;
        await page.goto(url, { waitUntil: "domcontentloaded" });
        await page.waitForLoadState("networkidle");
        // Beat for the cache seed effect + any URL-flag mutation.
