// scripts/preview-calendar.mjs — Calendar preview pipeline (PR 3I).
//
// Same shape as the prior runners: build prod, boot vite preview,
// confirm /_preview/calendar/* is unreachable in prod; then start vite
// DEV and screenshot each fixture across desktop + mobile. Output →
// docs/design/redesign/previews/calendar-3i.pdf.
...
const outPdf = resolve(outDir, "calendar-3i.pdf");

const PORT = 4195;
const BASE = `http://localhost:${PORT}`;

const FIXTURE_PATH = resolve(repoRoot, "src/pages/_preview/fixtures/calendar.js");
...
async function verifyProd404() {
  console.log("Verifying prod build /_preview/calendar is unreachable…");
  ...
  try {
    ...
    await page.goto(`http://localhost:${prodPort}/_preview/calendar/calendar-populated-month`, {
      waitUntil: "networkidle",
    });
    const bodyText = await page.evaluate(() => document.body.innerText);
    await browser.close();
    // Calendar-specific markers. If any appear in prod, the DEV gate
    // has failed.
    const MARKERS = [
      "Your career on a calendar.",
      "Every task due date, application, and interview",
      "Follow-up / Networking",
    ];
    if (MARKERS.some((m) => bodyText.includes(m))) {
      throw new Error(
        "Prod /_preview/calendar check FAILED — Calendar rendered in production. The DEV gate isn't working.",
      );
    }
    console.log("Prod build /_preview/calendar unreachable (falls through to AuthenticatedApp -> /login).");
  } finally {
...
async function main() {
  ...
  try {
    ...
    if (ids.length === 0) throw new Error("No calendar fixtures loaded.");
    console.log(`Loaded ${ids.length} calendar fixtures: ${ids.join(", ")}`);
    ...
    for (const vp of VIEWPORTS) {
      ...
      for (const id of ids) {
        ...
        const url = `${BASE}/_preview/calendar/${id}`;
        await page.goto(url, { waitUntil: "domcontentloaded" });
        await page.waitForLoadState("networkidle");
        // Beat for the post-mount DOM-action driver (view-mode pill /
