...
//   - 183 roles with id, title, alternate_titles
...
const ROLE_LIBRARY = resolve(ROOT, "supabase/functions/_shared/libraries/00_role_library.ts");
const ROLE_SKILL_MAP = resolve(ROOT, "supabase/functions/_shared/libraries/04_role_skill_mapping.ts");
...
const withMappings = roles.filter((r) => r.core_skills.length > 0 || r.secondary_skills.length > 0).length;
...
console.log(`Wrote ${roles.length} roles (${withMappings} with skill mappings) to src/lib/roleSkillsGenerated.json`);
