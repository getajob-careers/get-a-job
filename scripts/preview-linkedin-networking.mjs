// scripts/preview-linkedin-networking.mjs — LinkedIn 3J-C (Networking)
// preview pipeline. Loads only fixtures whose ids start with
// `linkedin-networking-` and writes
// docs/design/redesign/previews/linkedin-3jc.pdf.
...
const outPdf = resolve(outDir, "linkedin-3jc.pdf");
...
function loadFixtureIds() {
  ...
  while ((m = re.exec(src)) !== null) {
    if (!m[1].startsWith("linkedin-networking-")) continue; // 3J-C subset only
    ids.push(m[1]);
    labels[m[1]] = m[2];
  }
  return { ids, labels };
...
async function verifyProd404() {
  ...
  try {
    ...
    await page.goto(
      `http://localhost:${prodPort}/_preview/linkedin/linkedin-networking-list-empty`,
...
async function main() {
  ...
  try {
    ...
    if (ids.length === 0) throw new Error("No linkedin-networking-* fixtures loaded.");
    console.log(`Loaded ${ids.length} networking fixtures: ${ids.join(", ")}`);
    ...
    for (const vp of VIEWPORTS) {
      ...
      for (const id of ids) {
        ...
        // Comment-coach + composer fixtures depend on chained post-mount
        // actions (input fill → generate → result). Give them extra
        // time before the screenshot.
        const settleMs =
          id.includes("comment-coach") ||
          id.includes("composer-target-form") ||
          id.includes("composer-goal-picker")
            ? 1500
            : 900;
