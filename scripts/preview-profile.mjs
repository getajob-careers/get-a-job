// scripts/preview-profile.mjs — Profile preview pipeline (PR 3F).
//
// Same shape as the prior runners: build prod, boot vite preview,
// confirm /_preview/profile/* is unreachable in prod; then start vite
// DEV and screenshot each fixture across desktop + mobile. Output →
// docs/design/redesign/previews/profile-3f.pdf.
...
const outPdf = resolve(outDir, "profile-3f.pdf");

const PORT = 4185;
const BASE = `http://localhost:${PORT}`;

const FIXTURE_PATH = resolve(repoRoot, "src/pages/_preview/fixtures/profile.js");
...
async function verifyProd404() {
  console.log("Verifying prod build /_preview/profile is unreachable…");
  ...
  try {
    ...
    await page.goto(`http://localhost:${prodPort}/_preview/profile/profile-populated`, {
      waitUntil: "networkidle",
    });
    const bodyText = await page.evaluate(() => document.body.innerText);
    await browser.close();
    // Profile-specific markers. If any of these appear in prod, the
    // DEV gate has failed — the harness leaked into the production
    // bundle.
    const MARKERS = [
    ...
    if (MARKERS.some((m) => bodyText.includes(m))) {
      throw new Error(
        "Prod /_preview/profile check FAILED — Profile rendered in production. The DEV gate isn't working.",
      );
    }
    console.log("Prod build /_preview/profile unreachable (falls through to AuthenticatedApp -> /login).");
  } finally {
...
async function main() {
  ...
  try {
    ...
    if (ids.length === 0) throw new Error("No profile fixtures loaded.");
    console.log(`Loaded ${ids.length} profile fixtures: ${ids.join(", ")}`);
