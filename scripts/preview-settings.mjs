// scripts/preview-settings.mjs — Settings (PR 3N) preview pipeline.
// Loads `settings-*` fixtures → docs/design/redesign/previews/settings-3n.pdf.
...
const outPdf = resolve(outDir, "settings-3n.pdf");

const PORT = 4199;
const BASE = `http://localhost:${PORT}`;

const FIXTURE_PATH = resolve(repoRoot, "src/pages/_preview/fixtures/settings.js");
...
function loadFixtureIds() {
  ...
  while ((m = re.exec(src)) !== null) {
    if (!m[1].startsWith("settings-")) continue;
    ids.push(m[1]);
    labels[m[1]] = m[2];
  }
  return { ids, labels };
...
async function verifyProd404() {
  console.log("Verifying prod build /_preview/settings is unreachable…");
  ...
  try {
    ...
    await page.goto(
      `http://localhost:${prodPort}/_preview/settings/settings-default`,
    ...
    if (MARKERS.some((m) => bodyText.includes(m))) {
      throw new Error(
        "Prod /_preview/settings check FAILED — Settings rendered in production. The DEV gate isn't working.",
      );
    }
    console.log("Prod build /_preview/settings unreachable (falls through to AuthenticatedApp -> /login).");
  } finally {
...
async function main() {
  ...
  try {
    ...
    if (ids.length === 0) throw new Error("No settings-* fixtures loaded.");
    console.log(`Loaded ${ids.length} settings fixtures: ${ids.join(", ")}`);
    ...
    for (const vp of VIEWPORTS) {
      ...
      for (const id of ids) {
        ...
        const url = `${BASE}/_preview/settings/${id}`;
        await page.goto(url, { waitUntil: "domcontentloaded" });
        await page.waitForLoadState("networkidle");
        // password-done fixture has nested setTimeouts (compose →
        // verify → done). Give it more time to settle.
        const settle = id.includes("password-done") ? 1800 : id.includes("password-verify") ? 1200 : 900;
