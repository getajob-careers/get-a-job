...
//
// EXISTING_MATCHER_PROMPT below is a verbatim copy of the system prompt
// that lived inline in match-internship-companies/index.ts at line 258
// before PR4. Captured at extraction time. If you change the shared
// prompt, you must intentionally update this constant — the test
// failure WILL surface drift.

const EXISTING_MATCHER_PROMPT = `You are an internship strategy advisor for "Get A Job," a career operating system for early-career professionals entering tech roles. The student has a PITCH STRATEGY (internship_profile) describing what KINDS of companies are realistic for them, what role archetypes they can pitch given today's strengths, and how this internship serves their long-term career.
...
describe("prompt identity gate — shared module must match the existing matcher prompt byte-for-byte", () => {
  it("PITCH_SYSTEM_PROMPT_BASE === captured matcher prompt", () => {
    expect(PITCH_SYSTEM_PROMPT_BASE).toBe(EXISTING_MATCHER_PROMPT);
  });

  it("buildSystemPrompt({ includeWhoToContact: false }) === matcher prompt (unchanged path)", () => {
    expect(buildSystemPrompt({ includeWhoToContact: false })).toBe(EXISTING_MATCHER_PROMPT);
  });
  ...
  it("buildSystemPrompt({ includeWhoToContact: true }) appends the who_to_contact extension", () => {
    ...
    expect(withExt.startsWith(EXISTING_MATCHER_PROMPT)).toBe(true);
    expect(withExt.length).toBeGreaterThan(EXISTING_MATCHER_PROMPT.length);
