// send-reengagement-email — ONE-OFF re-engagement to onboarding-INCOMPLETE users
// (confirmed email, not unsubscribed, not internal), inviting them to finish the
// rebuilt V2 onboarding. DRY-RUN by default: renders + logs to email_dry_run_log,
// never sends. Invoked by the hub / a manual workflow_dispatch, not a schedule.
//
...
import { renderReengagementEmail } from "../_shared/email-templates.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const recipients = await resolveEligibleRecipients(svc, "reengagement");
    ...
    console.error("send-reengagement-email error:", (e as Error).message);
