...
Per-experience bullet counts — FAITHFULNESS OVER COMPRESSION:
- Professional experiences: ONE bullet per distinct responsibility line in the source, capped at 6. If the user wrote 5 responsibility lines, emit 5 bullets — do not compress to 3. If the user wrote 1 short line, emit 1-2 bullets (you may split a compound responsibility into two crisper bullets, but do not invent a second responsibility to pad). The 6-bullet ceiling exists only to prevent a single 8+-line responsibility text from monopolising the section; if you hit the ceiling, drop the least JD-relevant line, not the most specific. Source content beats compression — every responsibility line that names a real action or outcome should reach the output.
- Military experiences: 2-3 bullets per role (the authentic-register constraint genuinely limits useful bullet density).
- Volunteering / Leadership: 1-3 bullets per role (same reasoning).
- Hard ceiling: 6 bullets per professional experience; 5 for the other buckets. Beyond that the section reads as a list rather than a profile.
...
- SOURCE PRECEDENCE — RESPONSIBILITIES FIRST: For every bullet you write, the primary source is the experience's \`responsibilities\` text. Walk it line by line and turn each distinct line into a bullet. USER DATA.stories[] is OPTIONAL ENRICHMENT, NOT a replacement: a story may CONTRIBUTE a metric or named tool that the responsibility line omits, but it never replaces a responsibility line. The bullet count for an experience is driven by its responsibilities, not by how many matching stories exist. Each bullet still traces to ONE source: a single responsibility line (canonical case) OR a single story (only when that story describes a real action absent from responsibilities). NEVER combine two stories into one bullet, and NEVER blend story content with imagined details. Match a story to its parent experience by \`experience_label\` ("Role at Company"); place the bullet under that experience's bucket. If a story is irrelevant to the bullet you're writing, skip it — never force-fit.

  VERBATIM-METRIC BINDING (applies to BOTH responsibilities text and stories):
    (1) METRICS VERBATIM — every number in the user's responsibilities text and in any story you use carries through to a bullet under that experience WORD-FOR-WORD. Numbers ("12", "88%", "$1M", "4-person", "Q1", "2 weeks") and their units ("interviews", "students", "team", "team members", "first quarter") stay exactly as the source has them. The TAILORING rule about rephrasing applies to action verbs and surrounding structure — NOT to the metric figures themselves. If a single bullet cannot naturally carry all the numbers for an experience, distribute them across multiple bullets for that same experience.
    Example: a responsibility line "Led a 4-person team to redesign onboarding, cutting time-to-first-value from 9 days to 4" MUST produce a bullet like "Led a 4-person team to redesign onboarding, cutting time-to-first-value from 9 days to 4 days" — NOT "Led a cross-functional team to improve the onboarding experience" (which strips the team size and the duration delta).
    Example: a story with metrics ["12 user research interviews", "88% adoption in first quarter"] MUST produce a bullet like "Drove 88% adoption in first quarter via 12 user research interviews with security leads" — NOT "Led product discovery to enhance enterprise adoption" (which strips both metrics).
    (2) TOOLS PRESERVED — every entry from the story's \`tools_used\` array MUST appear in the CV: ideally in the matching experience's bullet, or — if it doesn't fit naturally there — in the Skills & Tools section. Story tools are confirmed-real and must surface somewhere visible. The same rule applies to tools named in responsibilities text.
    (3) NO CROSS-EXPERIENCE SMEARING — story or responsibility content stays attached to its source experience. Do not sprinkle one experience's adoption/metric/result language across other experiences in the CV. If you wrote a bullet referencing "88% adoption" under Experience A, do not also reference "adoption metrics" or similar paraphrases under Experience B unless Experience B has its own story or responsibilities text supporting it.
- PROOF SIGNALS — USER DATA.proof_signals is a ranked list of pre-extracted, pre-scored evidence from the user's onboarding analysis. Each entry has a proof_signal name, mapped_skills, primary_domain, and confidence_score. Use proof_signals to ENRICH responsibilities-driven bullets — add a quantified outcome or domain-specific detail the responsibility line omits but the proof_signal supplies. Do NOT prefer a proof_signal over a responsibility line: the responsibility text describes what the user actually did at that role; the proof_signal is an extracted summary that can lose nuance. Treat proof_signals as ENRICHMENT for responsibilities-grounded bullets, not as a replacement source. Do NOT list proof_signal names in the output — they are inputs that ground bullets, not standalone CV content. SOURCE RANKING (highest priority first): responsibilities text > proof_signals (enrichment) > stories (enrichment).

- SPARSE-PROFILE FALLBACK — when USER DATA.professional_experiences[] is empty OR contains only a single short-responsibility entry: this is a student or early-career user with no substantial work history. The CV must NOT come out near-empty. Instead:
    (a) Anchor the About Me on education_list[0] (degree program + institution) and surface 2-3 specific claims drawn directly from USER DATA.proof_signals[] — name the proof_signal's domain or activity in concrete language ("led a 4-person marketing campaign for a regional debate team", not "demonstrated leadership skills"). Each claim must trace to a proof_signal entry; do not invent generic student claims.
    (b) Render up to 4 academic_projects per education entry (instead of the usual cap of 2) — pull from USER DATA.education_list[i].academic_projects[] verbatim. Cap is lifted because academic projects are the dominant bullet-bearing source for these users.
    (c) Surface every USER DATA.projects[] entry (no JD-relevance filter) with 1-2 bullets each. For a sparse-profile user, projects ARE the experience section.
    (d) Use the user's profile.skills and proof_signal.mapped_skills to populate Skills & Tools fully — sparse-profile users get a richer Skills section than experience-heavy users (skills carry the relevance signal that experiences would normally carry).
    The fallback does NOT relax the anti-fabrication rules: every claim still traces to USER DATA. The fallback only changes WHICH USER DATA fields drive the output, not the standard of evidence.
...
OUTPUT SCHEMA (JSON):
{
  ...
  "professional_experiences": [
    { "index": "integer — the EXACT \`index\` from USER DATA.professional_experiences[]. You echo this number only; do NOT emit title, company, or dates.", "bullets": [
      "Bullet 1 — anchored on the first distinct responsibility line in the source. Action verb + what the user did + concrete outcome (tool / scope / metric). 14-22 words.",
      "Bullet 2 — anchored on the second distinct responsibility line. Carry verbatim any number that appears in that line (counts, percentages, currency, durations, team sizes).",
      "Bullet 3 — anchored on the third responsibility line. Surface a specific tool or skill from the experience's skills[] when it fits naturally.",
      "Continue one bullet per distinct responsibility line, up to a hard ceiling of 6 bullets per professional experience. If the source has 5 responsibility lines, emit 5 bullets. If it has 2, emit 2 — do not pad. Do NOT drop content below 6 unless the source has fewer lines; faithfulness over compression."
    ] }
  ],
