...
The whole journey, in plain English — what a student actually experiences, from the first sign-up to landing interviews. No code here; for the technical version, see [engineering](../engineering/architecture.md).

## 1. Sign up and onboard

A new student signs up (the pilot uses invite codes, so access is gated to the cohort). Onboarding is where the product learns who they are:
...
By the end, the student has a profile that reflects their real history and a first read on which roles they should be chasing.
...
The app maintains a fresh database of real open jobs at Israeli companies (thousands of them, refreshed nightly from company career systems). The **Career** page shows live jobs filtered to the student's tracks, each with a match score, so they're applying to real, relevant openings — not scrolling a generic board.

## 4. Build strong materials

When a student targets a role, the product helps them prepare:

- **Tailored CV** — a CV rewritten for that specific job, drawing on the student's own captured stories so the achievements are real, not invented.
...
A guiding rule runs through all of it: **nothing is fabricated.** The AI only uses what the student has actually told it.

## 5. Run the hunt

- **Tracker** — every application, its status, and what needs doing next, on a board the student manages.
...
An **AI career agent** sits across the whole product. It knows the student's pipeline, roadmap, and stories, and answers questions, drafts messages, and recommends what to focus on — like a career coach who has read everything and never gets tired.
