...
*Every living doc here shows an owner and a "last reviewed" date, and is checked for staleness by `npm run docs:check`. See the [documentation guide](meta/documentation-guide.md) for how that works. If you find something out of date, fixing it is part of the change that made it stale — not a separate chore.*
