...
**The entire tail is one line: the Pass-2 call. It is NOT post-processing and NOT non-LLM** (my intermediate "~34s non-LLM" reading was wrong too — corrected here). Everything after Pass 2 — reconcile, anti-fab, page-fit — is sub-millisecond; PDF 0.7s; storage+persist ~2s. The Pass-2 **call bracket** is 31s on OpenRouter Sonnet but **6.8s on direct-OpenAI gpt-4o** for the same authoring work. So the ~24s excess is **specific to the OpenRouter Sonnet transport**, not the model's inherent decode (a direct provider call of the same size finishes in ~7s).
...
## 5. Re-ranked levers (measured — the tail is the OpenRouter Sonnet transport)

The whole game is the Pass-2 OpenRouter Sonnet call (~24s excess vs a direct provider). The pre-measurement levers (prompt cache, Pass-1 downgrade, streaming) target the ~5s that already works and are **dominated**; streaming keeps perceived-latency value only.

| #      | Lever                                                                                                 | Est. saving           | Risk                                                                  | Status                                                                                                                                                           |
| ------ | ----------------------------------------------------------------------------------------------------- | --------------------- | --------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1a** | **Sonnet via direct Anthropic API** (bypass OpenRouter), env-selectable, OpenRouter kept as fallback  | **~24s → total ~15s** | Med — needs Anthropic key + a transport wrapper; verify output parity | **Built, held — PR #633.** Merges keyless; activation = `supabase secrets set ANTHROPIC_API_KEY` + `CV_PASS2_TRANSPORT=anthropic`. Waiting on Anthropic billing. |
| **1b** | **OpenRouter provider pinning** (`OPENROUTER_PROVIDER_PREFS`: pin fast provider, drop slow fallbacks) | partial → most of 24s | Low-med — stays on OpenRouter; magnitude unproven until measured      | **Built, held — PR #634.** The this-week bridge while 1a waits on billing. Ships only if a measurement shows pinning helps.                                      |
| 2      | Confirm A-vs-B (hidden-retry vs trickled-body)                                                        | diagnostic            | trivial                                                               | **Folded into #633** as `pass2_headers`/`pass2_body` marks. Provisional verdict **B** (consistency).                                                             |
| 3      | `extract-job-requirements` 401 (grounding correctness, not latency)                                   | 0 latency             | separate                                                              | **Fixed, held — PR #632.** service-role invoke; stage-3 grounding had 401'd since 2026-05-26.                                                                    |
| —      | _Old levers:_ prompt-cache ~0s, Pass-1 downgrade ~1–2s, streaming perceived-only                      | dominated             | —                                                                     | Suspended. Streaming remains a future perceived-latency candidate.                                                                                               |

## 6. Fix sequence (as built)

1. **#632 (stage-3 401 fix)** — rides the first merge+deploy; independent of the transport question.
2. **#634 (1b provider pinning)** — the bridge: measure pinning effect; if it recovers a meaningful chunk of the 24s, ship now so users get faster generations this week.
3. **#633 (1a direct-Anthropic)** — the durable win; merges keyless now, activates on billing unstick (one secret + flag flip). Verification on activation: harness before/after (target ~15s p50 vs 39.3s baseline) + output-parity spot-check (index-validity must survive the transport swap) + SPEED GUARD table.
