...
- **origin/main tip = `d275482`.** PR-D (`afe69c3`) + all THREE pre-cert PRs (#819, #821, #824) are MERGED + LIVE. Frontend-only, NO edge deploy this whole arc. (CV lane also merged past PR-D: #818/#820 etc. - unrelated to design; verify the tip on resume.)
...
- **#824 (squash `d275482`)** - two flag-on cosmetic tweaks from Eli's live re-look:
  - **Seamless canvas gutters.** On viewports wider than CanvasShell's `max-w-[1400px]`, the body bg (white `--background`) showed in the side gutters. Fix: one UNLAYERED rule `:root[data-next-design] body { background-color: var(--rd-bg-page); }` in `index.css` -> body propagates cream (`#f4ebda`) to the viewport gutters, seamless with the shell. Flag-OFF byte-identical (no attr). Login/onboarding paint their own full-width `bg-rd-bg-page` so unaffected. Ground wash untouched (separate `-z-10` `background-image` layer, no body-color dependency).
  - **Tile order (Eli ruling).** `TOOL_TILES` reordered to Tasks, Interview coach, LinkedIn, Home, Profile, Career, Story bank, Skill hub, Coach (Coach = the full-page CareerAgent tile, LAST because the Coach dock is always in the rail). Desktop + mobile rails share the one array. Remainder (Career, Story bank, Skill hub, Coach) was my proposed judgment - STANDS unless Eli objects at his glance.
- Verification (all three PRs): Spec PASS, Flag-Scope all CONFIRMED, QA-Breaker no P0/P1/P2, gate GREEN (typecheck 518 net-0, 1822 tests). VERIFICATION blocks on each PR. All hub-verified PASS then merged.
...
- **#824 (live):** wide-desktop (>1400px) flag-on gutters read as seamless canvas, not white (also re-confirms the subtle ground-wash-in-frame vs flat-gutter boundary reads as ONE canvas); the toolkit tile order + remainder (Career, Story bank, Skill hub, Coach) reads right at a glance - Eli can re-rule the remainder if he wants a different order.
...
**LANE IDLE. NOTHING queued to BUILD.** All pre-cert items - 1 sort-disclosure, 2 persona copy, 3 horizontal pan, the homepage-relabel rider (#819/#821), the wide-viewport canvas gutters + ruled tile order (#824) - are DONE + LIVE. The proposed remainder tile order (Career, Story bank, Skill hub, Coach) STANDS unless Eli objects at his glance. The next design-lane work is **Flip 2 execution** (the reveal cutover), which starts **only when Eli certs** the outstanding eyeballs above and rules #723 clear. Until then, wait for Eli. Do NOT touch the reveal flag or auth-config autonomously.
