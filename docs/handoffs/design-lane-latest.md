...
- **ONE design PR HELD: #797** `eli/onboarding-back-nav` (tip `dc645f4`). **Item 10 back-nav, RULED Option A + provenance fix.** One shell-level Back on OnboardingV2 for `step >= 2` (direction 2->1, springboard 3->2); removed the screen-1 review->upload back (StepReview back gated on `onBack`, V1 keeps it). **UNCONDITIONAL both flag states.** 3 ruled items PASS / flag-scope PASS / gate GREEN. **The exposed provenance deviation is FIXED (Eli RULED b, `dc645f4`):** `buildReviewProfilePayload`/`persistReviewProfile` now take `extractedPrimaryDomain`; the `'extracted'` stamp fires only when the written domain EQUALS the raw extracted domain (`domainsMatch`), so a back-nav-backfilled INFERRED domain stays `'inferred'`. Re-verified PASS (round-trip fixed, CV-extracted + CV-less-forward unregressed; test `onboardingReviewPersist.test.js` 10/10). **STILL HELD: `persistOnboardingProfileV2.js` is a CV-LANE owned persist path -> awaits CV-LANE CROSS-REVIEW of the persist-file diff (the HUB routes it). Do NOT merge.** On CV-lane sign-off: merge via ritual (no `--delete-branch`; `gh api DELETE` ref). Live V2 walkthrough folds into item-11 functional QA.
...
3. **Item 10 - CLOSED pass3 (only gate = CV-lane review of #797).** Task 3 RESOLVED (SMTP ~20-min lag; draw order RETIRED). Situation min-1 RULED OPTIONAL (no build). Back-nav RULED Option A -> BUILT + provenance fix RULED (b) applied + re-verified in #797. ONLY remaining thread = **CV-LANE CROSS-REVIEW of the `persistOnboardingProfileV2.js` diff** (hub-routed); on sign-off, merge #797 via ritual. Nothing else for the design lane here.
