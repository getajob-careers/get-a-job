...
**Last updated: 2026-06-04 (tracker-board-only — Tracker is now a single kanban view. Removed List|Board toggle, `?view=kanban` URL param, and status-filter pills; kanban columns serve as the implicit status filter. Board cards now carry the Track badge and "may be inactive" warning that used to live only on list rows. Fixed two board bugs at the same time: drag failed to initiate and clicks did nothing — both caused by ApplicationKanbanCard being a native `<button>` child of a Draggable, which @hello-pangea/dnd's interactive-element guard refuses to drag (the button swallowed pointer-down before DnD registered drag intent). Card now renders as `<div role="button">` with onKeyDown — same pattern as `src/components/internship/CompanyTargetCard.jsx`. ApplicationRow gained a `defaultExpanded` prop so kanban-driven expansion opens the row already-expanded instead of collapsed. All seven application statuses, optimistic-mutate on `["applications", uid]` cache key, and the `trg_log_application_status_change` audit-trigger contract preserved 1:1. Previously: generate-tasks-dedup.).**
...
| `src/pages/Tracker.jsx` | **Board-only** (was List+Board until 2026-06-04). Single kanban view across 7 status columns; clicking a card opens the full `ApplicationRow` detail (tabs, locks, writes) below the board. List view, `?view=kanban` URL param, and status-filter pills all removed — columns serve that role |
