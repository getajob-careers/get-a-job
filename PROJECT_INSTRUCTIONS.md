...
**Last updated: 2026-06-11 (seamless-IA — Sidebar slims to Today / Career / Chat / Internship / Profile. Home rebuilt as the **Today** command center: header date + greeting, stat strip (Live matches / In pipeline / Tasks done), coral focus card with the daily action, Today's plan + Pipeline (4-bucket funnel: saved / applied / interview / offer) side by side, coach band at the bottom. New `src/pages/Career.jsx` condenses Roadmap + Jobs onto one page: track band re-filters both panes, live-jobs pane with scoreJobFit match badges + low-fit dimming + Track/Apply buttons, matched-roles "why" rail with two axis bars (Qualified now + Moves you to {goal}) and matched/gap skill chips. Old pages (Roadmap, Jobs, Tracker, Tasks, Calendar, StoryBank, Linkedin, Resources) stay registered in `pages.config.js` and remain deep-linkable — they just left the sidebar. Design rulings now live in `docs/design/redesign/isaac/RULINGS.md` and are part of the spec: (a) one SCORE per card maximum; counts and fractions are exempt; (b) explanatory axes render as bars with NO numerals; (c) no UI string asserts anything without a grounding data source; (d) low-fit provenance reads "shown for completeness, not recommended" (trigger-honest); (e) null score columns never render as 0% — omit. The PNG mockups in that directory are visual direction only; conflicts with RULINGS.md resolve to the ruling. Typecheck count: 404 (10 below the pre-merge main, no new errors). Previously: feedback-widget.).**
