...
**Last updated: 2026-06-04 (generate-tasks-dedup — `supabase/functions/generate-tasks/index.ts` now (a) pre-queries the user's most-recent 50 completed tasks, (b) injects them into the system prompt with a "do not regenerate semantic equivalents" directive (soft enforcement), (c) drops post-parse any LLM-emitted task whose stopword-stripped lowercase title matches a completed title (hard backstop via new shared helper `_shared/task-dedup.ts`: `normalizeTaskTitle` / `dedupeAgainstHistory` / `capTasksByPriority`), and (d) priority-sorts + caps at MAX_TASKS=8 so a low-first LLM ordering can't drop all high-priority items. Adds an anti-convergence TITLE PERSONALIZATION RULE forbidding stock titles ("Define target roles", "Update your CV") and requiring user-specific anchors (role names, skill_gaps, named applications). Server-side only; Tasks.jsx replace-flow + Onboarding.jsx fire-and-forget caller unchanged. Onboarding first-run no-ops cleanly (empty history). Edge fn does NOT auto-deploy — `supabase functions deploy generate-tasks --project-ref ilmqmodklutztuybsvwd` required post-merge. Previously: workday-detail-fetch.).**
