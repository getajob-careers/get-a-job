...
**Last updated: 2026-06-05 (transfer-map-coverage — Filled the deterministic transfer-matrix coverage gap. `15_skill_transfer_map.ts` had ZERO transfer entries for 25 roles, so `computeGoalAlignment` in `generate-career-analysis` returned near-zero scores for any source/target involving them. The 25 zero-coverage set covered all 12 #247 additions (compliance_analyst, chief_compliance_officer, compliance_counsel, legal_counsel, paralegal, senior_legal_counsel, vp_legal, research_scientist, medical_science_liaison, process_engineer, manufacturing_quality_engineer, quality_control_technician, production_technician, hardware_engineer) + 13 pre-existing gaps (IT_Security cluster: devsecops/IR/threat_hunter/security_researcher; Finance ops: bookkeeper/senior_bookkeeper/payroll_accountant; sales_enablement_manager; ai_creative_specialist; customer_success_operations + renewals_manager from #355). Added 111 curated transfer entries across 10 clusters with `type` overrides (17 natural / 59 stretch / 35 pivot) grounded in role_library `next_roles` / `similar_roles` and known IL-tech transitions. Also fixed pre-existing stats drift — the `total_transfer_pairs` / `by_type` / `total_roles_with_mappings` numbers at the top of the file were stale before this edit; resynced to actual array contents. Entries set `s`, `t`, `sen_gap`, `type` + `curated: true` + `curated_note`; placeholder `score`/`shared`/`gaps`/`gap_n`/`gap_d` defaults a future scorer run can recompute. Consumed by `generate-career-analysis` only (single edge function); needs manual `supabase functions deploy generate-career-analysis` after merge. _shared/libraries edit — cross-review by Isaac per CLAUDE.md required. Previously: tracker-board-detail-drawer.).**
