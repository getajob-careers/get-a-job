...
**Last updated: 2026-06-06 (feedback-widget — Floating "Beta: got feedback?" pill mounted in `src/Layout.jsx` on every authenticated, post-onboarding route. Opens a `Dialog` modal with a two-step flow: pick category chip (Bug / Confusing / Missing feature / Other) → write a description → Send. Submits direct from the browser via the supabase anon client to a new `public.feedback` table — no edge function indirection, no Slack/email ping in v1. New analytics event `EVENTS.FEEDBACK_SUBMITTED` fires on successful insert with `{category, route}` for PostHog cohorting. Table count 35 → 36. Categories are deliberately broad so the pilot doesn't pre-commit to surface-specific buckets before users tell us where complaints cluster — refine from "Other" later. RLS model: users INSERT + SELECT their own; `is_admin(auth.uid())` reads all (for /admin triage). Migration `20260605_feedback_table.sql` must be applied manually via Supabase MCP `apply_migration` before merge — repo's migrations do NOT auto-apply on PR merge, and the widget will 42P01 if the table is missing in prod. Previously: transfer-map-coverage.).**
