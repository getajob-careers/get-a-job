...
| IA-3b / PR-A2 | Career — inline pipeline board + `/Tracker` redirect + first-time guide | complex | ☐ | Final Tracker-absorption PR. Strip is now a clickable toggle that expands an inline section between the strip and the jobs/rail two-pane block. Section mounts `<ApplicationsKanban>` + `<ApplicationDetailDrawer>` exactly as `Tracker.jsx` does — same canonical statuses, status-labels, `inactiveExternalIds` cross-ref query (gated on `boardOpen && atsLinkedKeys.length > 0` so it doesn't fire when the board is closed), single `TRACKER_CSS` injection to preserve `.tk-*` class consumers inside the drawer tabs. URL contract is real (params were pre-wired no-ops by PR-A1): `?pipeline=open` renders the board expanded + scrolls it into view via `requestAnimationFrame` + `scrollIntoView({ behavior: "smooth" })`; `&app=<id>` opens the detail drawer on that application (closing the drawer drops `app` but keeps `pipeline=open`). All param mutations go through `setSearchParams(..., { replace: true })` — no nav, no history pollution. Collapse state persists for the session only (URL-only; no localStorage). `/Tracker` route becomes `<Navigate to="/Career?pipeline=open" replace>` via a small eager `src/pages/TrackerRedirect.jsx`, swapped into `pages.lazy.js`'s `LAZY_PAGES.Tracker`; `?app=<id>` passthrough preserved. `Tracker.jsx` and `TrackerPreview` left untouched — kanban regression checks still mount the real Tracker via `/_preview/tracker/:state`. First-time guide: the 4-tile 7-step framing card moves from `Tracker.jsx` into the expanded board as a dismissible card; copy updates "tracker" → "pipeline" where it reads naturally; dismissal persists in localStorage under `pipelineGuideDismissed:<uid>` (Home `focusDone` precedent). Cleanup rider: `funnelBuckets.js` comment fixed — `rejected` is the sole exclusion (`withdrawn` isn't part of the enum). Deviation: the Tracker manual-add dialog (paste a JD, type a title, set status) is deferred to a follow-up PR — re-introducing the shadcn Dialog primitive in a new file added 4 new TS errors of the same repo-wide `forwardRef`-in-JSX pattern that AddEventDialog and Tracker.jsx already have (typecheck ratchet rule kicks in). JobCard Track button remains the primary add path; `ApplicationDetailDrawer` covers full editing of an existing row. Preview: `docs/design/redesign/previews/career-board.pdf` (board-collapsed, board-expanded-populated, board-expanded-empty, drawer-open, guide-dismissed × 1280 + 390). Branch: `eli/career-pipeline-board`. |
