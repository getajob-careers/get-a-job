...
| 3N | Settings | simple | ☑ | Restyled `Settings.jsx` + `PasswordCard.jsx` (hex → rd tokens; slab H1 + section headings; coral CTAs; danger zone on rd-primary palette). **HIGHEST-STAKES PAGE** — className/JSX only, zero behavior change. P41–P46 byte-equivalent: two-step password flow (auth.reauthenticate → updateUser({password,nonce})); delete-account edge fn + DELETE_CONFIRM_PHRASE gate + case-insensitive match + logout(false) + `?deleted=1` redirect; reset_user_data RPC + two-state resetConfirming gate + localStorage.removeItem + 8 queryClient.removeQueries; all inline error surfaces. PasswordCard prop signatures untouched (`password-card.test.jsx` 617/617 holds). Deferred per Eli: email promotion, password-modal collapse, lowercase-phrase, sidebar. |
...
| IA-3e | Coach docked in sidebar (always visible) + mobile header trigger, shared state with panel | complex | ☐ | Founder design revision after seeing both right-edge-tab and sidebar-nav-item approaches: the Coach is not a launcher but a **persistent docked live chat** in the sidebar's dead space below the nav items on desktop, plus a **persistent header trigger** on mobile. New `src/lib/CoachConversationContext.jsx` lifts the chat state (messages, input, sending, conversation_id, conversation list, sendMessage with 401-refresh-retry + page_context forwarding, applied-action sets) out of `ChatInterface` for the drawer surface so the dock and the panel are **one conversation, two views** — anything sent in either appears instantly in the other. New `src/components/agent/CoachDock.jsx` mounts in `Layout`'s sidebar between the nav and `SidebarFooter` (flex-1 min-h-0); slim header (Sparkles + "Coach" + expand icon → panel), compact `CoachThread`, persistent `CoachInput` ("Ask about this page…"). Short-viewport collapse via `ResizeObserver` below ~220px (thread hidden, header + input only, expand still opens panel). Scroll contract: thread region is its own scroll container with `overscroll-behavior: contain`; sidebar is already `fixed lg:static` full-height (no fix needed). Narrow-width audit: `MessageBubble` gains a `variant` prop (no fork) — dock variant tightens density (gap-2, smaller avatar, max-w-92%, px-3 py-2, text-[12px]). Suggested-action cards render in condensed single-row form (`Suggestion — {summary} >`) with an "Open in panel" expand path. `AgentDrawer` panel rewired: replaces `ChatInterface variant="drawer"` with the same shared `CoachThread variant="panel"` + `CoachInput variant="panel"` reading from the provider. Panel mechanics (desktop right 520px, mobile bottom 85vh, z-[58]/[60] over `ApplicationDetailDrawer` Sheet z-50, seed integration — seed pre-populates the shared input via `conv.setInput`) unchanged. Mobile: new `MobileCoachTrigger.jsx` — coral chip (Sparkles + "Coach") in Layout's mobile top header beside the hamburger; PRIMARY mobile entry. Dock also renders inside the opened hamburger sidebar as the SECONDARY path — sending or expanding from it opens the bottom sheet. Right edge stays clear (no floating elements competing with job-card Apply buttons or the feedback pill). The Coach sidebar nav-item attempt from #295 is reverted (no top-level Coach in `BASE_SECTIONS`). Chat nav group untouched (full-page agents remain the deep entry). `ai-chat` and all server prompts UNTOUCHED (a bake-off is asserting byte-equality against production in parallel). DEV preview at `/_preview/drawer/:state` rewired for: dock-idle, dock-with-conversation, dock-thread-scrolled-mid (independent-scroll proof), dock-collapsed-short-viewport, panel-expanded-from-dock, mobile-header-trigger-visible, mobile-bottom-sheet-open, mobile-hamburger-sidebar-with-dock. Preview pdf: `docs/design/redesign/previews/agent-entry-sidebar.pdf` (8 captures). Deviation: SUGGESTED_*_JSON action card application is deferred — the dock + panel surface the condensed suggestion row + expand link; the rich card "Apply" handlers will follow in a Phase-B card-extraction PR. Branch: `eli/agent-entry-sidebar` (force-pushed). |
| IA-3e-attempt-1 | Coach as a sidebar nav item, retire right-edge tab (superseded by the docked rework) | simple | ✗ | Founder decision after production use: the right-edge "Ask your agent" floating tab from PR-A3 was too invisible. The agent entry moves into the left sidebar as a first-class top-level "Coach" nav item (Sparkles icon for surface continuity with Home's coach band), positioned directly under Today + above Career. Coach is NOT a route — `onClick` calls `useAgentDrawer().open()`; active-state styling (`bg-rd-primary-tint text-rd-primary-dark` + coral dot) applies while the panel is open. `SidebarSection` gains a third render branch (`action: "open-agent-drawer"`) parallel to the existing `section.page` (Link) and `section.items` (collapsible group) branches. `Layout` split into outer wrapper (mounts `AgentDrawerProvider`) + inner `LayoutBody` that consumes `useAgentDrawer` for the active-state read. Mobile: Coach is reachable via the existing hamburger → sidebar tap-through (one extra tap vs the right-edge tab); the Coach click closes the mobile sidebar before opening the drawer so the sheet animation isn't competing with the sidebar slide-in. Lean documented in the PR body — separate mobile trigger (e.g., header button) is a quick follow-up if pilot data shows the sidebar tap-through is friction. AgentDrawer.jsx: right-edge tab JSX deleted, MessageCircle import dropped, `open` no longer destructured (sidebar owns the trigger). Panel mechanics (desktop right-side 520px panel, mobile bottom 85vh sheet, z-[58]/[60] overlay/panel above ApplicationDetailDrawer's z-50 Sheet, ChatInterface variant + seed mechanics, rolling-conversation mount strategy) UNCHANGED from PR-A3 / PR-B2. ai-chat edge function NOT touched (a prompt bake-off is asserting byte-equality against production in parallel). Chat nav group stays untouched (full-page agents remain the deep entry until the Phase B merge). `DrawerPreview` rewired to mount inside the real `Layout` so the sidebar Coach entry is visible. New states: entry-visible, panel-open-from-sidebar, panel-open-with-seed, panel-open-over-detail-sheet, mobile-sidebar-with-coach. Preview pdf: `docs/design/redesign/previews/agent-entry-sidebar.pdf` (9 captures). Branch: `eli/agent-entry-sidebar`. |
| IA-3d / PR-B2 | Agent page-context injection (Phase B2) — server-authoritative + drawer wiring + rate bump | complex | ☑ | Phase B2 of the agent drawer rollout. The drawer now forwards a `page_context` object on every turn: `{page, application_id?, job_id?, role_id?, track?, company_target_id?}` — IDs only, no titles/content. Server sanitizes (whitelist enum / UUID-format / valid keys), fetches each entity scoped to `auth.uid()` (`career_roles.user_id`, `company_targets.user_id`, `jobs` public-read), and injects structured blocks parallel to the existing TARGET APPLICATION pattern: `CURRENT PAGE`, `CURRENT TRACK`, `TARGET ROLE`, `TARGET JOB`, `TARGET COMPANY`. Unknown / foreign IDs silently drop — never errored to the LLM. New `supabase/functions/ai-chat/page-context.ts` with `sanitizePageContext` + `fetchPageContextEntities` + `renderPageContextBlocks`; pure render fn produces `""` on empty input so byte-equivalence with the legacy assembly path is testable (`page-context.test.ts` covers 22 cases including the byte-equivalence contract). `application_id` plumbing: when page_context carries it and the body's top-level `application_id` is absent, the existing TARGET APPLICATION fetch path is used (no duplicate block in renderPageContextBlocks). Legacy callers (CVAgent / InterviewCoach / SkillAdvisor with explicit `application_id`) keep working untouched. `RATE_LIMIT_CALLS` bumped 30 → 50 same window — documented as the drawer-volume allowance. New `useAgentDrawer().pageContext` + `setPageContext(ctx)` slot; shallow-equal guard prevents render cascades. ChatInterface accepts `pageContext` prop + threads it into both invoke sites (main + retry). Per-page wiring: Career sets `{page: "Career", track, role_id?, application_id?}` from `(selectedTrack, effectiveExpandedId, drawerAppId)`; Today/Profile/Calendar set `{page: …}` only; Internship sets `{page: "Internship", company_target_id?}` from `openTarget?.id`. Builder for Career extracted to a pure helper + tested (`src/test/career-page-context.test.js`, 7 cases). No schema migration (verified). Telemetry: `page_context_keys` added to function_metrics so Langfuse traces show which IDs the user sent each turn. Branch: `eli/agent-page-context`. |
...
- `src/components/tracker/ApplicationChecklist.jsx` — rebuilt as a
  ...
  **Know the role** (steps 1–2, golden), **Build your case**
  ...
  progress bar (`completedCount / 7` × `--rd-primary`). Same 7
...
- `src/pages/Calendar.jsx` — Tailwind + rd tokens directly. `<style>{ACT_CSS}</style>`
  ...
  use rd-primary-tint border / golden-tint hover / opacity respectively.
...
on the page (app chrome, CTAs, refine sparkles) is rd-primary.
...
- `src/components/chat/ChatInterface.jsx` — orchestrator restyled.
  ...
  uniform white-card surface with rd-primary icon accents. Applied
...
- `src/components/chat/AgentIntro.jsx` — collapsible intro panel
  restyled (rd-bg-card border, rd-primary-tint how-to-use highlight).
...
- `src/components/chat/StorySaveCard.jsx` — migrated from violet
  ...
  white-card surface with rd-primary icon + rd-teal-tint saved
...
| `--rd-primary` | `#EF5A41` | primary CTA accent |
| `--rd-primary-dark` | `#C7461F` | coral hover / emphasis |
| `--rd-primary-tint` | `#FCE6DF` | coral-on-cream backgrounds |
