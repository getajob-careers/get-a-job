...
---
2026-07-28 - Stale local origin/main can silently revert a sibling PR's just-merged fix in a shared file
Trigger: email arc. After server-side squash-merging #830 (an em-dash copy fix in the SHARED email-templates.ts) and #831, I updated the still-open #834 by merging my LOCAL origin/main - which had NOT been fetched since before those merges, so it was stale. The merge brought an OLD email-templates.ts and silently reverted #830's fix on #834's side; the em dashes REAPPEARED. Caught only because I re-grepped the shared file and saw the U+2014 come back.
What I did wrong: treated "merge origin/main" as if the local ref tracked the server. gh pr merge advances origin server-side but NOT the local origin/main until git fetch; merging a stale base silently undoes another PR's change to any file both branches touch - with NO conflict to warn you.
Rule for next time: before updating ANY open branch that touches a file another just-merged PR also touched, `git fetch origin main` FIRST, merge the FRESH origin/main, then RE-VERIFY the shared file (grep the exact lines the sibling PR changed) before pushing. A silent revert produces no conflict marker; only the re-grep catches it.
---
